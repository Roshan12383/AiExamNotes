...
function Pricing(){
    ...
    const[payingAcount,setPayingAcount]=useState(null);


    const handlePaying=async(amount)=>{
     try {
        setPayingAcount(amount)
        setPaying(true)
        ...
        <div className="min-h-screen bg-gray-100 px-6 py-10 relative">
            ...
            <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
                 <PricingCard  
                  ...
                  onBuy={handlePaying}
                  paying={paying}
                  payingAmount={payingAcount} />


                   <PricingCard  popular
                   title="Popular" price="₹200" amount={200} credits="120 Credits"
...
title,price,amount,credits,description,features,popular,selectedPrice,setSelectedPrice,onBuy,
paying,payingAmount
}){
    ...
    return(
        ...
        ${isSelected ? "border-black":popular ? "border-indigo-500":"border-gray-200"}`}>
         ...
         {popular && !isSelected && <span className="absolute top-4 right-4 text-xs px-2 py-1 rounded bg-indigo-600
        ...
        {isSelected && <span className="absolute top-4 right-4 text-xs px-2 py-1 rounded bg-black text-white">
         ...
         ${isPayingThisCard?"bg-gray-300 cursor-not-allowed":isSelected ? "bg-black text-white"
